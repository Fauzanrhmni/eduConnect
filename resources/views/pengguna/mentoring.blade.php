    ...
    <div class="flex flex-col gap-5 w-[20%] mt-20">
        ...
        <div class="flex w-full justify-center items-center pr-5 border-r-blue-400 border-r-[3px] ml-[2.8px] z-20">
            <a href="{{ route("pengguna.mentoring") }}" class="w-full bg-white flex flex-row items-center gap-3 p-5 rounded-xl shadow-sm">
                <div>
                    <img src="{{ asset("assets/briefcase-blue.svg") }}" alt="">
                </div>
                <h2 class="text-blue-400 font-medium text-base">Mentoring</h2>
            </a>
        </div>
            ...
            <div class="flex flex-row w-full justify-center items-center gap-5">
                <a href="#" onclick="openModal('modalTambah')" class="flex justify-center items-center w-full bg-white rounded-xl pr-2 py-2 pl-5 shadow-sm">
                    <div class="flex flex-row justify-between items-center w-full">
                        <p class="text-base">Tambah Mentoring baru</p>
                        <div class="flex items-center justify-center p-3 bg-blue-500 bg-blue-600 rounded-xl h-full">
        ...
        <div class="flex flex-col gap-5">
            ...
            @if($mentorings->isEmpty())
                <div class="w-full p-8 bg-white flex items-center justify-center rounded-xl shadow-sm mb-32">
                    <h1 class="text-2xl font-semibold m-auto">Belum Ada Mentoring!!</h1>
            ...
            <div class="w-full p-8 bg-white flex flex-col rounded-xl shadow-sm">
                <h1 class="text-2xl font-semibold w-full mb-4">{{ $mentoring->judul }}</h1>
                    ...
                    <div class="flex flex-row justify-center items-center">
                        <div class="relative flex items-center justify-center p-3">
                            <div class="flex items-center justify-center overflow-hidden rounded-lg h-16 w-16">
                                <img src="{{ $mentoring->user->image ? asset('storage/' . $mentoring->user->image) : asset('assets/user_regis.svg') }}" alt="" class="object-cover object-top h-full w-full">
                            </div>

                            <div class="absolute flex p-1 items-center justify-center left-0 bottom-0 bg-slate-700 rounded-md">
                                @if ($mentoring->user->role == 1)
                        ...
                        <div class="flex flex-col">
                            <h4 class="text-base font-semibold">{{ $mentoring->user->name }}</h4>
                            <h3 class="text-sm text-gray-400">{{ $mentoring->created_at->diffForHumans() }}</h3>
                        </div>
                    </div>

                    <div class="flex flex-row items-center gap-3">
                        @if ($mentoring->status == 'tersedia')
                            ...
                            <div class="flex flex-row px-3 py-2 h-fit justify-center text-sm items-center bg-green-200 text-green-500 border-2 font-medium border-green-500 rounded-lg">
                                {{ ucfirst($mentoring->status) }}
                                
                            </div>

                        @else

                            <div class="flex flex-row px-3 py-2 h-fit justify-center text-sm items-center bg-red-200 text-red-500 border-2 font-medium border-red-500 rounded-lg">
                                {{ ucfirst($mentoring->status) }}
                            </div>

                        @endif

                        <div class="flex flex-row px-3 py-2 h-fit justify-center text-sm items-center bg-blue-200 text-blue-500 border-2 font-medium border-blue-500 rounded-lg">
                            {{ ucfirst($mentoring->keahlian) }}
                        </div>
                    </div>
                </div>

                <div class="flex items-center flex-row gap-3 text-xl mt-7">
                    <img src="{{ asset("assets/peserta.svg") }}" alt="" class="h-6">
                    {{ $mentoring->total_peserta }} Peserta
                </div>

                <p class="w-full text-base text-gray-500 break-words whitespace-pre-line">
                    {{ $mentoring->deskripsi }}
                </p>

                <div class="flex flex-row items-center mt-5 gap-5">
                    <form action="{{ route('mentoring.favorit', $mentoring->id) }}" method="POST">
                        @csrf
                        @php
                            $isFavorit = auth()->user() && auth()->user()->favoritMentorings->contains($mentoring->id);
                        @endphp
                    ...
                    @if ($mentoring->user_id == auth()->id())
                        {{-- Tombol Edit jika user yang membuat mentoring --}}
                        <button onclick="openEditModal({{ $mentoring->id }}, '{{ $mentoring->judul }}', '{{ $mentoring->keahlian }}', '{{ $mentoring->status }}', '{{ $mentoring->link }}', '{{ $mentoring->deskripsi }}')" class="flex items-center gap-2 p-3 bg-yellow-400 hover:bg-yellow-500 text-white rounded-xl">
                            <img src="{{ asset('assets/edit.svg') }}" alt="Edit" class="h-4">
                            Edit mentoring
                        </button>

                        <button type="button" onclick="openModal('hapusMentoringModal{{ $mentoring->id }}')">
                          <i class="fas fa-trash p-4 bg-red-500 hover:bg-red-600 rounded-lg text-white flex items-center justify-center"></i>
                        </button>

                        <!-- Modal Konfirmasi Hapus mentoring -->
                        <div id="hapusMentoringModal{{ $mentoring->id }}" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 hidden">
                          <div class="bg-white rounded-lg p-6 w-[90%] max-w-lg">
                            ...
                            <p class="text-center text-gray-600 mb-4">
                              Apakah Anda yakin ingin menghapus mentoring <strong>{{ $mentoring->judul }}</strong>?
                            </p>
                            <div class="flex justify-center gap-4">
                              <form action="{{ route('pengguna.mentoring.destroy', $mentoring->id) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="bg-red-500 hover:bg-red-600 text-white px-5 py-2 rounded-md">Hapus</button>
                              </form>
                              <button onclick="closeModal('hapusMentoringModal{{ $mentoring->id }}')" class="bg-gray-400 hover:bg-gray-500 text-white px-5 py-2 rounded-md">Batal</button>
                            </div>
                          </div>
                        </div>

                        {{-- Ganti tombol jika user login adalah pembuat mentoring --}}
                        <div class="flex items-center gap-2 text-gray-500">
                            <img src="{{ asset('assets/check.svg') }}" alt="">
                            <span class="text-sm">You created this mentoring</span>
                        </div>

                    @else

                        {{-- Tampilkan tombol Add mentoring --}}
                        <a href="{{ $mentoring->link }}" target="_blank" class="flex flex-row items-center justify-center p-3 bg-blue-500 hover:bg-blue-600 gap-2 text-white rounded-xl">
...
<div id="modalTambah" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 hidden">
  <div class="bg-white rounded-2xl py-6 px-10 w-full max-w-lg">
    ...
    <form class="mt-4 flex flex-col gap-2" action="{{ route('pengguna.mentoring.store') }}" method="POST">
      @csrf
      <label class="text-sm">Judul Mentoring</label>
      <input type="text" name="judul"
          ...
          placeholder="Judul Mentoring" required />
      ...
      <textarea name="deskripsi" class="px-5 py-3 border shadow-sm bg-blue-50 placeholder-gray-500 focus:outline-none focus:border-blue-500 focus:ring-blue-500 block w-full rounded-lg sm:text-sm focus:ring-1" placeholder="Masukan Deskripsi Mentoring" required></textarea>
...
<div id="modalEdit" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 hidden">
  <div class="bg-white rounded-lg p-6 w-[90%] max-w-lg">
    <h1 class="text-xl font-bold text-center text-blue-500">
      Edit Mentoring
    </h1>

    <form class="mt-4 flex flex-col gap-2" id="formEditMentoring" action="" method="POST">
      @csrf
      @method('PUT')
      <input type="hidden" name="id" id="editId">

      <label class="text-sm">Judul Mentoring</label>
      <input type="text" name="judul" id="editJudulMentoring"
      ...
      <label class="text-sm">Keahlian Mentoring</label>
      <input type="text" name="keahlian" id="editKeahlian"
        class="px-3 py-3 bg-white border shadow-sm border-slate-300 placeholder-slate-400 focus:outline-none focus:border-blue-500 focus:ring-blue-500 block w-full rounded-md sm:text-sm focus:ring-1"
        placeholder="Keahlian Mentoring" required />

      <label for="status" class="text-xs">Status</label>
      <select name="status" id="editStatus"
        class="mt-1 px-3 py-3 bg-white border shadow-sm border-slate-300 focus:outline-none focus:border-blue-500 focus:ring-blue-500 block w-full rounded-md sm:text-sm focus:ring-1">
        <option value="tersedia">Tersedia</option>
        <option value="penuh">Penuh</option>
      </select>
      ...
      <textarea name="deskripsi" id="editDeskripsi" class="px-3 py-3 bg-white border shadow-sm border-slate-300 placeholder-slate-400 focus:outline-none focus:border-blue-500 focus:ring-blue-500 block w-full rounded-md sm:text-sm focus:ring-1" placeholder="Masukan Deskripsi Mentoring" required></textarea>

      <button type="submit"
        class="bg-blue-500 px-5 py-2 rounded-md font-bold mt-1 w-full text-center text-white">
        Simpan
      </button>
      <button type="button"
        class="bg-white px-5 py-2 border-2 border-blue-500 rounded-md font-bold mt-1 w-full text-center text-blue-500"
...
<script>
  ...
  function openEditModal(id, judul, keahlian, status, link, deskripsi) {
      document.getElementById('editId').value = id;
      document.getElementById('editJudulMentoring').value = judul;
      document.getElementById('editKeahlian').value = keahlian;
      document.getElementById('editStatus').value = status;
      document.getElementById('editLink').value = link;
      document.getElementById('editDeskripsi').value = deskripsi;

      let formEdit = document.getElementById('formEditMentoring');
      formEdit.action = `{{ url('pengguna/mentoring/update') }}/${id}`;
