...
<div class="px-4 md:px-10 mx-auto w-full -m-24">
        <div class="flex flex-wrap">
            <div class="w-full mb-4 px-4">
                <div class="relative flex flex-col min-w-0 px-2 pt-2 pb-4 break-words w-full mb-6 shadow-lg rounded-2xl bg-white">
                  <div class="flex flex-row items-center justify-center gap-3 absolute right-5 top-5">
                    @if(session('success'))
                      <div class="bg-green-100 text-green-800 p-3 rounded text-xs">
                          {{ session('success') }}
                      </div>
                    @endif

                    <a href="#" onclick="openModal()" class="text-blue-500 text-xs flex flex-row items-center justify-center gap-2 pr-1.5 pl-2 py-1 bg-white hover:bg-slate-100 font-semibold rounded-lg">
                  ...
                  <div class="-mt-16 ml-5 flex flex-col lg:flex-row gap-3 lg:gap-20">
                      <div class="flex flex-col">
                          <img src="{{ $user->image ? asset('storage/' . $user->image) : asset('assets/user.svg') }}" alt="User Profile" class="rounded-full h-32 w-32 object-cover bg-white p-2">
                      ...
                      <div class="justify-end flex flex-col w-full lg:w-1/2 gap-3 pr-5">
                        <div class="flex flex-col lg:flex-row gap-3">
                            ...
                            <div class="px-3 py-2 flex flex-row justify-start items-center gap-3 text-sm border-2 rounded-md w-full">
                                <i class="fas fa-briefcase text-blueGray-400 mt-1"></i>
        ...
        <div id="myModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 hidden">
          <div class="bg-white rounded-lg p-6 w-[90%] max-w-lg">
              ...
              <!-- Form di dalam modal -->
              <form class="mt-4 flex flex-col gap-2" action="{{ route('admin.profile.update') }}" method="POST"
